...
class Category(Base):
    ...
    SubCategory: Mapped[List["SubCategory"]] = relationship(
        "SubCategory", back_populates="Category_"
    )
...
class ProductBrand(Base):
    ...
    ProductBrandID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
    ...
    Product: Mapped[List["Product"]] = relationship(
        "Product", back_populates="ProductBrand"
    )
...
class ProductColor(Base):
    ...
    ProductColorID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
    ...
    Product: Mapped[List["Product"]] = relationship(
        "Product", back_populates="ProductColor"
    )
...
class ProductDepartment(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "ProductDepartmentID", name="PK__ProductD__DFE96784B2871EA9"
        ),
    )

    ProductDepartmentID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
    ...
    Product: Mapped[List["Product"]] = relationship(
        "Product", back_populates="ProductDepartment"
    )
...
class ProductImage(Base):
    ...
    ProductImageID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
    DateCreated: Mapped[datetime.datetime] = mapped_column(
        DateTime, server_default=text("(getdate())")
...
class Size(Base):
    ...
    SizeID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
    ...
    ProductSize: Mapped[List["ProductSize"]] = relationship(
        "ProductSize", back_populates="Size"
    )
...
class Users(Base):
    ...
    SearchHistory: Mapped[List["SearchHistory"]] = relationship(
        "SearchHistory", back_populates="Users"
    )
...
class SearchHistory(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["UserID"], ["Users.UserID"], name="FK__SearchHis__UserI__07C12930"
        ),
        PrimaryKeyConstraint("SearchHistoryID", name="PK__SearchHi__555F7C991720DCE6"),
    )

    SearchHistoryID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
    SearchInput: Mapped[str] = mapped_column(
        Unicode(255, "SQL_Latin1_General_CP1_CI_AS")
    )
    DateCreated: Mapped[datetime.datetime] = mapped_column(
        DateTime, server_default=text("(getdate())")
...
class SubCategory(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["CategoryID"],
            ["Category.CategoryID"],
            name="FK__SubCatego__Categ__46E78A0C",
        ),
        PrimaryKeyConstraint("SubCategoryID", name="PK__SubCateg__26BE5BF9F7AD0B9D"),
    )

    SubCategoryID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
    SubCategoryName: Mapped[str] = mapped_column(
        Unicode(255, "SQL_Latin1_General_CP1_CI_AS")
    )
    DateCreated: Mapped[datetime.datetime] = mapped_column(
        DateTime, server_default=text("(getdate())")
    ...
    Category_: Mapped["Category"] = relationship(
        "Category", back_populates="SubCategory"
    )
    Product: Mapped[List["Product"]] = relationship(
...
class ProductLabel(Base):
    __tablename__ = "ProductLabel"
    __table_args__ = (
        PrimaryKeyConstraint("ProductLabelID", name="PK__ProductL__605E50FC253D7107"),
    )

    ProductLabelID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
    LabelName: Mapped[str] = mapped_column(Unicode(50, "SQL_Latin1_General_CP1_CI_AS"))

    Product: Mapped[List["Product"]] = relationship(
        "Product", back_populates="ProductLabel"
    )
...
class Product(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            name="FK__Product__Product__7E37BEF6",
        ),
        ForeignKeyConstraint(
            ["ProductColorID"],
        ...
        ForeignKeyConstraint(
            ...
            name="FK__Product__Product__7F2BE32F",
        ),
        ForeignKeyConstraint(
            ["ProductImageID"],
            ["ProductImage.ProductImageID"],
            name="FK__Product__Product__7C4F7684",
        ),
        ForeignKeyConstraint(
            ["SubCategoryID"],
            ["SubCategory.SubCategoryID"],
            name="FK__Product__SubCate__7B5B524B",
        ),
        ForeignKeyConstraint(
            ["ProductLabelID"],
            ["ProductLabel.ProductLabelID"],
            name="FK__Product__Product__123EB7A3",
        ),
    ...
    ProductCaption: Mapped[str] = mapped_column(
        Unicode(255, "SQL_Latin1_General_CP1_CI_AS")
    )
    ProductPrice: Mapped[decimal.Decimal] = mapped_column(DECIMAL(8, 2))
    NewPrice: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(8, 2))
    DateCreated: Mapped[datetime.datetime] = mapped_column(
        DateTime, server_default=text("(getdate())")
    )

    ProductBrand: Mapped["ProductBrand"] = relationship(
        "ProductBrand", back_populates="Product"
    )
    ProductColor: Mapped["ProductColor"] = relationship(
        "ProductColor", back_populates="Product"
    )
    ProductDepartment: Mapped["ProductDepartment"] = relationship(
        "ProductDepartment", back_populates="Product"
    )
    ProductImage: Mapped["ProductImage"] = relationship(
        "ProductImage", back_populates="Product"
    )
    SubCategory: Mapped["SubCategory"] = relationship(
        "SubCategory", back_populates="Product"
    )
    ProductSize: Mapped[List["ProductSize"]] = relationship(
        "ProductSize", back_populates="Product_"
    )
    ProductLabel: Mapped["ProductLabel"] = relationship(
        "ProductLabel", back_populates="Product"
    )
    ...
    ProductDepartmentID = Column(
        Integer, ForeignKey("ProductDepartment.ProductDepartmentID")
    )
    ProductLabelID: Mapped[int] = mapped_column(Integer)
...
class ProductSize(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["ProductID"], ["Product.ProductID"], name="FK__ProductSi__Produ__02084FDA"
        ),
        ForeignKeyConstraint(
            ["SizeID"], ["Size.SizeID"], name="FK__ProductSi__SizeI__02FC7413"
        ),
        PrimaryKeyConstraint("ProductSizeID", name="PK__ProductS__9DADF571A548B0CF"),
    )

    ProductSizeID: Mapped[int] = mapped_column(
        Integer, Identity(start=1, increment=1), primary_key=True
    )
